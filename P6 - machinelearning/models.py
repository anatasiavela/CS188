...
class LanguageIDModel(Model):
	...
	def run(self, xs, y=None):
		...
		if not self.initialized:
			self.W_1 = nn.Variable(n, self.d)
			self.b_1 = nn.Variable(1, self.d)
			self.W_2 = nn.Variable(self.d, 5)
			self.b_2 = nn.Variable(1, 5)
			self.initialized = True

		g = nn.Graph([self.W_1, self.b_1, self.W_2, self.b_2])
		self.h = nn.Input(g, np.zeros((b, self.d)))
		count = 0
		for x in xs:
			c = nn.Input(g, x)
			mul1 = nn.MatrixMultiply(g, c, self.W_1)
			add1 = nn.MatrixVectorAdd(g, mul1, self.b_1)
			add2 = nn.Add(g, add1, self.h)
			self.h = nn.ReLU(g, add2)
			count += 1
			if count == len(xs):	
				add2 = nn.MatrixMultiply(g, self.h, self.W_2)
				add2 = nn.MatrixVectorAdd(g, add2, self.b_2)

		if Q_target is not None:
			inQ = nn.Input(g, Q_target)
			loss = nn.SoftmaxLoss(g, add2, inQ)
			return g
		else:
			return g.get_output(add2)
